...
[source,bash,linenums]
.Test Harness for `stats` Command
----
...
[options="nowrap"]
.Executing `stats` Command from the Terminal, Simulating Pipeline Input
----
...
single JSON object, but our `recent_tweets` command returns _multiple_
JSON objects. Don't worry, though; our `stats` command won't break
when we pipe our `recent_tweets` into it. On the server, Cog
