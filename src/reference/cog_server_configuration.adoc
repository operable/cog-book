...
+
Cog uses a set of common templates to format responses generated internally. Some of these templates may be overridden by placing a custom version in a directory and setting this var to said directory's path.
+
[TIP]
.Custom error templates
====
Currently the only overridable common template is the standard `error` template. See <<Customizing the standard error template>> for more information.
====
