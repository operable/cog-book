...
to HipChat instead of Slack, take a look at the alternative sections below:
  ...
  * <<Building_and_Running_Cog_from_Scratch,Building And Running Cog from Scratch>>
  * <<Connecting_Cog_to_HipChat,Connecting Cog to HipChat>>
...
Cog, skip down to the next section titled, <<Building_and_Running_Cog_from_Scratch,Building And Running Cog from Scratch>>.
...
---
image:images/max.png[Max,width=50,height=50,float="left"]
...
their descriptions for both <<Cog_Server_Configuration,Cog Server Configuration>> and <<Relay_Configuration,Relay Configuration>>.
...
Now you can head back to follow the section titled <<Creating_a_User_and_Running_a_Command,Creating a User and Running a Command>>,
