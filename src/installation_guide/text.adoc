...
on your own. In this guide, we'll walk you through:
  ...
  * Installing and running Cog
  * Connecting Cog to Slack
  * Installing and configuring a bundle
  * Authorizing and running a command pipeline

=== Running Cog with Docker Compose

We've provided both a `docker-compose.yml` and a
`docker-compose.override.example.yml` for running and configuring Cog. To run
the images specified in the `docker-compose.yml` you'll need to have both
...
Compose's installation guide], but I installed it with homebrew (`brew install
docker-compose`).

Next, we'll need to grab the `docker-compose.yml` and
`docker-compose.override.example.yml` from the Cog repo.

[source,bash]
----
curl -o docker-compose.yml \
  https://raw.githubusercontent.com/operable/cog/0.14.1/docker-compose.yml

curl -o docker-compose.override.yml \
  https://raw.githubusercontent.com/operable/cog/0.14.1/docker-compose.override.example.yml
----
...
[source,bash]
----
SLACK_API_TOKEN=xoxb-87931061512-4m0DshC79h8tLNjTMuxxozUo docker-compose up
----
...
section we'll create a Cog user associated with your Slack user and give it some
permissions, so you can start running some commands.
...
chat, Cog responseds with an error stating `Sorry, you aren't allowed to
execute 'operable:bundle list'`. That's because bundle commands require
...
@cog relay-group member add default heroku
----
...
both Erlang 18+ and Elixir 1.3.1+. You can find more information about how to
...
we'll just need a `SLACK_API_TOKEN` as everything else has a sensible default.
You can get a `SLACK_API_TOKEN` for your bot by creating a
https://my.slack.com/services/new/bot[new bot integration]. So let's try
...
Cogctl requires the same dependencies as Cog, Erlang 18+ and Elixir 1.3.1+.
...
----
./cogctl bootstrap
./cogctl relays create --id=$RELAY_ID --token=$RELAY_COG_TOKEN
----
